...
function Home() {
  ...
  return (
    <main className='container mx-auto px-4 my-4 text-center'>
      ...
      {posts.length > 0 && <p key={posts[count - 1].id + 'ps'} > <Link to={`post/${posts[count - 1].id}`} className='border-2 hover:bg-blue-400 border-white px-4 py-2 rounded'>OPEN POST: "{posts[count - 1].title}"</Link></p>}
